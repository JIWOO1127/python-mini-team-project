...
from src.ml import predictor_lgbm


def make_values():
    return {column: index for index, column in enumerate(predictor_lgbm.FINAL_COLS)}


def test_model_path_uses_received_pickle():
    assert predictor_lgbm.MODEL_PATH.name == "lgbm_battery_model.pkl"


def test_build_model_input_uses_float_final_column_order():
    model_input = predictor_lgbm.build_model_input(make_values())
    assert model_input.columns.tolist() == predictor_lgbm.FINAL_COLS
    assert model_input.shape == (1, len(predictor_lgbm.FINAL_COLS))
...
def test_build_model_input_rejects_missing_column():
    ...
    with pytest.raises(ValueError, match="manufacturing_year"):
        predictor_lgbm.build_model_input(values)
...
def test_load_vehicle_needs_no_brand_and_fills_missing_value(
    ...
    csv_path = tmp_path / "vehicles_lgbm.csv"
    pd.DataFrame([first, second]).to_csv(csv_path, index=False)
    monkeypatch.setattr(predictor_lgbm, "VEHICLE_DATA_PATH", csv_path)

    vehicle = predictor_lgbm.load_vehicle("ev0001")
...
def test_status_uses_failure_label_convention():
    assert predictor_lgbm._status_for_label(0) == "normal"
    assert predictor_lgbm._status_for_label(1) == "abnormal"
...
def test_failure_probability_returns_class_one_probability():
    probability = predictor_lgbm._failure_probability(
        ProbabilityModel(), predictor_lgbm.build_model_input(make_values())
    )
